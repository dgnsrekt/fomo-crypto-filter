...
class BittrexFilter:
    ...
    def __init__(self, currency_pair):
        ...
        self.exchange_data = ExchangeDatabase(BittrexFilter.NAME,
                                              BittrexFilter.COLUMNS,
                                              BittrexFilter.INDEX)

        self.dataframe = self.exchange_data.dataframes[self.currency_pair]
        print(self.dataframe.columns)

        self.volume_average = self.dataframe[BittrexFilter.VOLUME].mean()
...
while True:
    ...
    bittrex_btc_filter = BittrexFilter('ETH')
    bittrex_btc_filter.set_multipliers(volume=GLOBAL_VOLUME_MULTIPLIER)
    ...
    bittrex_btc_filter.set_thresholds()
    bittrex_btc_filter.filter_dataframe()
    print()
    print(bittrex_btc_filter)
    print(bittrex_btc_filter.dataframe)
    print()

    if len(bittrex_btc_filter) < 2:
        print('list < 2')
        print('reducing volume multiplier.')
        GLOBAL_VOLUME_MULTIPLIER /= 0.5
        # GLOBAL_PERCENT_SCORE_MULTIPLIER /= 0.5

    if len(bittrex_btc_filter) > 6:
        print('list > 6')
